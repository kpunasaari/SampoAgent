...
:root{{--surface-0:#090b14;--surface-1:#111526;--surface-2:#181d32;--surface-3:#222844;--border:#303858;--text:#f3f5ff;--muted:#aeb7d0;--primary:#8b7cff;--primary-hover:#a79cff;--success:#4fd6a8;--review:#ffc869;--risk:#ff7b91;--focus:#6db8ff;--radius:18px;--shadow:0 18px 45px rgba(0,0,0,.28)}}
...
.app-shell{{display:grid;grid-template-columns:248px minmax(0,1fr);min-height:100vh}}.sidebar{{position:sticky;top:0;height:100vh;padding:1.5rem 1rem;background:linear-gradient(180deg,#12172a,#0c0f1d);border-right:1px solid var(--border)}}.brand{{display:flex;gap:.7rem;align-items:center;padding:.45rem .65rem 1.7rem}}.brand-mark{{display:grid;place-items:center;width:34px;height:34px;border-radius:11px;background:linear-gradient(135deg,var(--primary),#45a6ff);font-weight:900}}.brand-copy strong,.brand-copy span{{display:block}}.brand-copy span{{font-size:.75rem;color:var(--muted)}}.sidebar nav{{display:grid;gap:.25rem}}.nav-link{{padding:.63rem .75rem;color:var(--muted);text-decoration:none;border-radius:10px;font-weight:650}}.nav-link:hover{{background:rgba(139,124,255,.13);color:var(--text)}}.nav-link.active{{background:linear-gradient(90deg,rgba(139,124,255,.28),rgba(109,184,255,.12));color:#fff;box-shadow:inset 3px 0 var(--primary)}}
...
form{{display:flex;flex-wrap:wrap;gap:.75rem;align-items:end;margin:.9rem 0}}label{{display:grid;gap:.32rem;min-width:150px;color:#d7ddf5;font-size:.86rem;font-weight:650}}input,select,button{{font:inherit;border-radius:10px;padding:.62rem .75rem}}input,select{{min-height:42px;background:#0c1020;color:var(--text);border:1px solid #414b71}}input:focus,select:focus,button:focus,a:focus{{outline:3px solid var(--focus);outline-offset:2px}}button{{border:1px solid transparent;background:linear-gradient(135deg,var(--primary),#6479ed);color:white;font-weight:750;cursor:pointer;box-shadow:0 8px 18px rgba(91,91,219,.22)}}button:hover{{filter:brightness(1.12)}}button.secondary{{background:var(--surface-3);border-color:#485276}}button.danger{{background:linear-gradient(135deg,#e84f70,#bd3856)}}.notice{{color:#dbe2fc;background:rgba(109,184,255,.1);border-left:3px solid var(--focus);padding:.7rem .85rem;border-radius:8px}}.metric{{font-size:1.8rem;font-weight:800}}.status{{display:inline-flex;align-items:center;gap:.35rem;border-radius:999px;padding:.25rem .58rem;font-size:.78rem;font-weight:800;white-space:nowrap}}.status-success{{color:#a7f3d6;background:rgba(79,214,168,.14)}}.status-review{{color:#ffdc91;background:rgba(255,200,105,.14)}}.status-risk{{color:#ffb3c0;background:rgba(255,123,145,.14)}}.action-row{{display:flex;flex-wrap:wrap;gap:.5rem;align-items:center}}.empty-state{{padding:1.5rem;text-align:center;color:var(--muted)}}
@media (max-width: 760px){{.app-shell{{display:block}}.sidebar{{position:static;height:auto;padding:1rem;border-right:0;border-bottom:1px solid var(--border)}}.brand{{padding:.2rem .3rem .8rem}}.sidebar nav{{display:flex;overflow-x:auto;padding-bottom:.25rem}}.nav-link{{white-space:nowrap}}.main-content{{padding:1.5rem 1rem}}section,.panel{{padding:1rem}}form{{display:grid}}label{{min-width:0}}button{{min-height:42px}}}}
</style></head><body><a class="skip-link" href="#content">Skip to content</a><div class="app-shell"><aside class="sidebar"><div class="brand"><span class="brand-mark">S</span><span class="brand-copy"><strong>SampoAgent</strong><span>Career workspace</span></span></div><nav aria-label="Main navigation">{nav}</nav></aside><main class="main-content" id="content"><header class="page-header"><p class="eyebrow">Career workspace</p><h1>{escape(title)}</h1></header>{body}</main></div></body></html>""")
