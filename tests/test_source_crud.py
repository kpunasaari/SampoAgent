...
def test_source_crud_and_delete_preserves_imported_job_snapshot():
    ...
    repository.update_source(source_id, name="Employer Finland", url="https://example.org/jobs.xml", country="Finland", source_type="employer career site", notes="Updated", capability="JSON Feed")
...
def test_source_ui_adds_edits_toggles_and_removes_sources(tmp_path):
    ...
    add = client.post("/sources", data={"name": "New board", "url": "https://example.org/jobs", "country": "Finland", "source_type": "job board", "capability": "RSS/Atom feed"}, follow_redirects=False)
    ...
    update = client.post(f"/sources/{source_id}/edit", data={"name": "Updated", "url": "https://example.org/feed.xml", "country": "Finland", "source_type": "job board", "capability": "JSON Feed", "notes": "JSON feed"}, follow_redirects=False)
